...
# Shuffle the dataset (optional but good practice, though we'll do a simple split)
# temp_list = list(zip(all_features, all_labels))
# random.shuffle(temp_list)
# all_features, all_labels = zip(*temp_list)
...
notebook.cells.append(nbformat.v4.new_code_cell(
    source="""# We need the knn_predict function defined in Part 3, Section 7.
# For this cell to work standalone if the notebook is run from here,
# we'd ideally re-import or ensure it's available.
# Assuming it's in the same notebook session:

# from pymath.lib.math import euclidean_distance, mode # Already imported in Part 3, Sec 7
# from collections import Counter # Already imported in Part 3, Sec 7

# (knn_predict function definition would be here if not run sequentially from top)
# For brevity, we assume it's available from previous cell execution in the notebook.
...
print("Notebook Part 4 appended successfully.")
print(f"Total cells in notebook now: {len(notebook.cells)}")

# For verification, let's print the source of the last 3 cells
print("\\n--- Last 3 cells for verification ---")
